...
def executeModel(m):
   ...
   df = pd.read_csv("../data/cpu-train.csv", na_values='?',delimiter=',')
   snip = df
   #snip = df[:12000]

   y = np.array(snip['cpu_01_busy'])

   snip = snip.drop('cpu_01_busy',1)
   snip = snip.drop('sample_time',1)
   snip = snip.drop('m_id',1)
   pdb.set_trace()

   X = np.array(snip)

   #df_test = pd.read_csv("../data/cpu-test.csv", na_values='?',delimiter=',')
   #tsnip = df_test
